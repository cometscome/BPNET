...
module chebyshev
  ...
  public :: chebyshev_polynomial, &
            ...
            chebyshev_polynomial_vector, &
            chebyshev_polynomial_d1_vector
...
contains
  ...
  pure function chebyshev_polynomial_vector(r, r0, r1, n) result(T)
   implicit none

   double precision, intent(in)        :: r(:), r0, r1
   integer,          intent(in)        :: n
   double precision, dimension(n+1, size(r)) :: T

   integer          :: i, j
   double precision :: x

   do concurrent (j = 1:size(r))
      T(:, j) = chebyshev_polynomial(r(j), r0, r1, n)
   end do

      !x = (2.0d0 * r(j) - r0 - r1) / (r1 - r0)

      !T(1, j) = 1.0d0
      !if (n > 0) then
      !   T(2, j) = x
      !   do i = 3, n+1
      !      T(i, j) = 2.0d0 * x * T(i-1, j) - T(i-2, j)
      !   end do
      !end if
   !end do

   end function chebyshev_polynomial_vector
 ...
 pure function chebyshev_polynomial_d1_vector(r, r0, r1, n) result(dT)
    ! Arguments:
    !    r        function argument
    !    r0, r1   the Chebyshev polynomials will be rescaled from [-1,1]
    !             to the interval [r0,r1]
    ...
    double precision, intent(in)     :: r(:), r0, r1
    integer,          intent(in)     :: n
    double precision, dimension(n+1, size(r)) :: dT

    integer::j

    do concurrent (j = 1:size(r))
      dT(:, j) = chebyshev_polynomial_d1(r(j), r0, r1, n)
   end do

  end function chebyshev_polynomial_d1_vector  
