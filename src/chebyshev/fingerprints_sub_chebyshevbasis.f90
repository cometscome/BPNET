submodule (fingerprints) chebyshevbasis
    use iso_fortran_env
    implicit none
    contains
    ...
    module subroutine read_basis_chebyshev(fp,line)
        ...
        call read_chebyshevdata(line,paramdata,fp%num_of_coeffs,fp%Rc_max )
        !r_Rc = paramdata(1)
        !a_Rc = paramdata(3) 

        !write(*,*) r_Rc,r_N,a_Rc,a_N
        !fp%num_of_coeffs = num_of_coeffs!r_N + a_N + 2
